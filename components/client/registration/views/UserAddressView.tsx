...
const UserAddressView = () => {
  ...
  const [date, setDate] = useState(new Date(new Date().getFullYear() - 15, 0, 1))
  ...
  const addAddressView = () => {
    return (
      <View style={styles.addressView}>
        <Input placeholder={t('address')} type={'fullStreetAddress'} inputStyle={[{ minWidth: '82%' }]} onClearInputText={() => addressRef.current.clear()} onChangeText={setOnSearchAddress} inputValue={onSearchAddress} value={onSearchAddress} onSubmitEditing={() => setIsVisible(false)} autoFocus />
    ...
    <>
      ...
      <View style={styles.inputContainer}>
        ...
        <View style={styles.innerContainer}>
          <Text style={[!profilePicture && { marginTop: getHeight(dimens.marginS) }]} title={t('add_profile')} />
          <TouchableOpacity activeOpacity={profilePicture ? 1 : 0.5} onPress={() => !profilePicture && setIsShowModal(true)} style={[styles.imageContainer, { marginLeft: getWidth(dimens.paddingXs) }]}>
            <Image source={profilePicture ? { uri: profilePicture } : require('assets/icon/editprofile.png')} style={profilePicture ? styles.selectedImage : styles.editProfile} />
          </TouchableOpacity>
          {profilePicture && (
            <TouchableOpacity activeOpacity={profilePicture ? 1 : 0.5} onPress={() => setIsShowModal(true)} style={[styles.imageContainer, { paddingLeft: getWidth(5) }]}>
              <Image source={require('assets/icon/circumEditBlue.png')} style={styles.editImage} />
            </TouchableOpacity>
          )}

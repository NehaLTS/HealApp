...
const LoginViewController = () => {
  ...
  const { setUserProfile, setUserId, setToken, userProfile } = UseClientUserContext()
  ...
  const validatePassword = () => {
    ...
    else if (!isValidPassword(passwordRef.current.value)) setPasswordError(`Password must have at least one special character(@#$!%*?&), one digit(0-9), one uppercase(A-Z)`)

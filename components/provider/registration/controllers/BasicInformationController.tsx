    ...
    if (currentStep.length !== totalSteps) {
      if (currentStep[currentStep.length - 1] === 0) {
        if (
          ...
          if (!userDataProvider.firstname?.length)
            setFirstNameError('First name is required');
          if (!userDataProvider.lastname?.length)
            setLastNameError('Last name is required');
          if (!userDataProvider.speciality?.length)
            setSpecialityError('Specialty is required');
          if (!userDataProvider.type_Provider?.length)
            setProviderTypeError('Type of provider is required');
          if (!userDataProvider.id_photo?.length)
            Alert.alert('Please Upload ID photo');
        }
      }
      if (currentStep[currentStep.length - 1] === 1) {
        if (
          userDataProvider.address &&
          userDataProvider.address.length >= 4 &&
          userDataProvider.phone_number &&
          ...
          if (!userDataProvider.phone_number?.length)
            setPhoneError('Phone number is required');
          if (!userDataProvider.address?.length)
            setAddressError('Address is required');
          else if (userDataProvider.address?.length)
            setAddressError('Please fill full address');
          if (!userDataProvider.license_photo?.length)
            Alert.alert('Please upload License photo');
        }
      }
    }
    if (currentStep[currentStep.length - 1] === 2) {
      if (
        userDataProvider.bank_name &&
        userDataProvider.branch &&
        userDataProvider.registration &&
        userDataProvider.account &&
        ...
        if (!userDataProvider.registration?.length)
          setRegistrationError('Registration is required');
        if (!userDataProvider.bank_name?.length)
          setBankNameError('Bank name is required');
        if (!userDataProvider.branch?.length)
          setBranchError('Branch name is required');
        if (!userDataProvider.account?.length)

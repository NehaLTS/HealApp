...
const RegistrationViewController = () => {
  ...
  const onPressSignUpProvider = async (email: string, password: string) => {
    ...
    setUserDataProvider({ ...userDataProvider, isSuccessful: response?.isSuccessful, provider_id: response?.user?.provider_id ?? '', token: response?.token ?? '' })
    console.log('response', response)
    if (response.isSuccessful)
      navigation.navigate(
        NavigationRoutes.ProviderBasicInfo
      )
    else {

...
const UserProfile = () => {
    ...
    const { userProfile, onLogoutButtonPress, paymentHistoryData } = UserProfileController()
    ...
    return (
        ...
        >
            <TouchableOpacity style={{ alignSelf: 'flex-start', marginBottom: getHeight(30) }} onPress={() => navigation.goBack()}>
                <Image source={arrowBack} style={styles.arrowBack} />
            </TouchableOpacity>
            <View style={{ width: '100%' }} >


                <Text title={`${'Name: '}${userProfile.firstName}`} style={styles.textStyle} />
                <Text title={`${'Wallet: '}${walletData?.wallet_amount ?? '0'}`} style={styles.textStyle} />
                <TextButton
                    title={'Add Payment'}
                    onPress={() => { setShowAddToWallet(!showAddToWallet) }}
                    fontSize={fontSize.textXl}
                    containerStyle={{ backgroundColor: colors.disabled, padding: getHeight(20), marginTop: getHeight(20) }}

                />
                {
                    showAddToWallet && <AddPaymentToWallet isShowInputView={true} />
                }
                <TouchableOpacity style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }} onPress={() => { setShowPaymentHistory(!showPaymentHistory) }}>
                    <Text title={'Payment history'} style={styles.textStyle} />
                    <Image source={arrowBack} style={[styles.arrowBack, { transform: [{ rotate: showPaymentHistory ? '270deg' : '180deg' }] }]} />

                </TouchableOpacity>
                {showPaymentHistory && <View style={{ height: getHeight(400), backgroundColor: colors.disabled }}>
